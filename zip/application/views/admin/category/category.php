                            ...
                            <div class="card-body">
                                ...
                                <div class="table-responsive">
                                    <table class="table table-striped table-bordered zero-configuration">
                                        ...
                                        <?php 
                                        if(count($category)!=0)
                                        foreach($category as $reg){ ?>
                                        <tbody>
                                            <tr>
                                                <td><?php echo $reg->title ;?></td>
                                                <td><?php echo $reg->arabic_title ;?></td>                                                
                                                <td><?php echo ($reg->status == 1)?"Enable":"Disable" ;?></td>
                                                <td><a href="<?php echo admin_url() . 'registration/edit_cat/' . $reg->id ?>" data-placement="top" data-toggle="popover" data-content="Edit" class="poper"><i class="fa fa-pencil text-primary"></i></a> / 
                                                    <?php if($reg->approve_status == 0){?>
                                                    <a href="<?php echo admin_url() . 'registration/status_cat/' . $reg->id .'/1'?>" data-placement="top" data-toggle="popover" data-content="Status" class="poper"><i class="fa fa-lock text-primary"></i></a><?php } else{ ?> <a href="<?php echo admin_url() . 'registration/status_cat/' . $reg->id .'/0'?>" data-placement="top" data-toggle="popover" data-content="Status" class="poper"><i class="fa fa-unlock text-primary"></i></a> <?php } ?>
                                                </td>
